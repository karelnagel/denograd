...
import {
  ...
  ScheduleItem,
  ScheduleItemContext,
} from "../jsgrad/engine/schedule.ts";
import {
  _Device,
  _MallocAllocator,
  Allocator,
...
const pyStr = async (o: any, useList = false): Promise<string> => {
  ...
  if (o instanceof Map) {
    return `{${
      (await Promise.all(
        [...o.entries()].map(async ([k, v]) =>
          `${await pyStr(k)}:${await pyStr(v)}`
        ),
      )).join(",")
    }}`;
  }
  if (o instanceof Set) {
    return `set([${
      (await Promise.all([...o].map(async (o) => await pyStr(o)))).join(", ")
    }])`;
  }
  ...
  if (o instanceof ImageDType) {
    return `tiny.dtype.dtypes.${o.name}(${await pyStr(o.shape)})${
      o.v !== 1 ? `.vec(${o.v})` : ""
    }`;
  }
  if (o instanceof PtrDType) {
    return `${await pyStr(o.base)}.ptr(${o.size}${
      o.local ? ", local=True" : ""
    })${o.v !== 1 ? `.vec(${o.v})` : ""}`;
  }
  if (o instanceof DType) {
    return `tiny.dtype.dtypes.${INVERSE_DTYPES_DICT[o.scalar().name]}${
      o.count > 1 ? `.vec(${o.count})` : ""
    }`;
  }
  ...
  if (o instanceof UPat) {
    ...
    const src = Array.isArray(o._in_src)
      ? (Array.isArray(o._in_src.at(0))
        ? new SkipFormatting(await pyStr(o._in_src.at(0), true))
        : o._in_src)
      : o._in_src;
    return t`tiny.ops.UPat(op=${o.op}, dtype=${o.dtype}, src=${src}, arg=${o.arg}, name=${o.name}, allow_any_len=${
      o.allowed_len === -1
    }, location=${o.location}, custom_early_reject=${o.custom_early_reject})`;
  }
  if (o instanceof UOp) {
    const arg = o.op === Ops.CONST && o.dtype === dtypes.float
      ? `float(${await pyStr(o.arg)})`
      : await pyStr(o.arg);
  ...
  if (o?.constructor?.name === "Object") {
    return `{${
      (await Promise.all(
        Object.entries(o).map(async (entry) =>
          await t`${entry[0]}:${entry[1]}`
        ),
      )).join(",")
    }}`;
  }
...
${
    data !== undefined
      ? `data = ${(await pyStr(data)).replaceAll("JS", "PYTHON")}`
      : ""
  }
def out(o):
    print("<<<<<"+to_ts(o)+">>>>>")
...
export const compare = <T extends any[] = any[]>(
  ...
  return () => {
    ...
    for (const [i, input] of inputs.entries()) {
      test(i.toString(), {
        skip: typeof options.skip === "boolean"
          ? options.skip
          : options.skip?.includes(i) || undefined,
      }, async () => {
