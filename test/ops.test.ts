...
Deno.test(
  ...
  test(
    [
      ...
      [UOp.const(dtypes.int, 3).add(UOp.float(4).idiv(UOp.float(55))).mul(UOp.int(3.4))],
      [UOp.const(dtypes.int, 3).add(UOp.float(4).idiv(UOp.float(55))).mul(UOp.boolean(false))],
      [UOp.const(dtypes.int, 3).add(UOp.float(4).idiv(UOp.boolean(false))).mul(UOp.int(3.4))],
      [new UOp({ op: Ops.IF, dtype: dtypes.bool, src: [new UOp({ op: Ops.CMPLT, dtype: dtypes.bool, src: [UOp.const(dtypes.int, 5), UOp.const(dtypes.int, 10)] }), UOp.const(dtypes.float, 1.0), UOp.const(dtypes.float, 0.0)] })],
    ],
    (x: UOp) => x.simplify(),
    'out(data[0].simplify())',
