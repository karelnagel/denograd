...
import {
  ...
  ClangRenderer,
  extra_pm,
} from "../jsgrad/renderer/cstyle.ts";
...
const ALL_PATTERN_MATCHERS: Record<string, PatternMatcher<any, any>> = {
  ...
  "tiny.codegen.rewriter.get_late_rewrite_patterns((Ops.EXP2, Ops.LOG2, Ops.SIN, Ops.AND, Ops.SHL, Ops.NEG, Ops.MULACC))":
    get_late_rewrite_patterns([
      Ops.EXP2,
      Ops.LOG2,
      Ops.SIN,
  ...
  "tiny.renderer.cstyle.ClangRenderer().extra_matcher":
    new ClangRenderer().extra_matcher,
  "tiny.renderer.wgsl.WGSLRenderer().string_rewrite":
    new WGSLRenderer().string_rewrite,
