...
export const MnistExample = () => {
  ...
  const train = async ([X_train, Y_train, X_test, Y_test]: Tensor[]) => {
    ...
    const test = async () => {
      const res = await new TinyJit(async () =>
        model.call(X_test).argmax(1).eq(Y_test).mean().mul(100)
      ).call();
  ...
  const maxDuration = steps.length
    ? Math.max(...steps.map((x) => x.duration!).filter(Boolean))
    : 0;
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      {currentStep && (
        <div>
          loss:{currentStep.loss?.toFixed(2)}, accuracy:{" "}
          {currentStep.acc?.toFixed(2)}, step:{" "}
          {currentStep.step}/{maxSteps}, step time:{" "}
          {round(currentStep.duration || 0)}s
        </div>
      )}
