...
import { bytes_to_string, concat_bytes, GlobalCounters, range, string_to_bytes, zip } from '../helpers.ts'
...
const fetchSave = async (url: string, path: string, dir: string, onProgress?: TqdmOnProgress) => {
  path = `${dir}/${path}`
  await env.mkdir(dir)
...
export class Llama3 {
  ...
  load = async (model_path: string, scale_dtype = dtypes.float16, device?: string | string[], onProgress?: TqdmOnProgress) => {
    // load weights
  ...
  prefill = async (toks: number[], start_pos = 0, device: string | string[], onProgress?: TqdmOnProgress) => {
  ...
  download = async (dir = `weights/llama3-${this.size}`, onProgress?: TqdmOnProgress) => {
    if (this.size === '1B') {
      await fetchSave('https://huggingface.co/bofenghuang/Meta-Llama-3-8B/resolve/main/original/tokenizer.model', 'tokenizer.model', dir, onProgress)
      return await fetchSave('https://huggingface.co/bartowski/Llama-3.2-1B-Instruct-GGUF/resolve/main/Llama-3.2-1B-Instruct-Q6_K.gguf', 'Llama-3.2-1B-Instruct-Q6_K.gguf', dir, onProgress)
    } else if (this.size === '8B') {
      await fetchSave('https://huggingface.co/bofenghuang/Meta-Llama-3-8B/resolve/main/original/tokenizer.model', 'tokenizer.model', dir, onProgress)
      await fetchSave('https://huggingface.co/TriAiExperiments/SFR-Iterative-DPO-LLaMA-3-8B-R/resolve/main/model-00001-of-00004.safetensors', 'model-00001-of-00004.safetensors', dir, onProgress)
      await fetchSave('https://huggingface.co/TriAiExperiments/SFR-Iterative-DPO-LLaMA-3-8B-R/resolve/main/model-00002-of-00004.safetensors', 'model-00002-of-00004.safetensors', dir, onProgress)
      await fetchSave('https://huggingface.co/TriAiExperiments/SFR-Iterative-DPO-LLaMA-3-8B-R/resolve/main/model-00003-of-00004.safetensors', 'model-00003-of-00004.safetensors', dir, onProgress)
      await fetchSave('https://huggingface.co/TriAiExperiments/SFR-Iterative-DPO-LLaMA-3-8B-R/resolve/main/model-00004-of-00004.safetensors', 'model-00004-of-00004.safetensors', dir, onProgress)
      return await fetchSave('https://huggingface.co/TriAiExperiments/SFR-Iterative-DPO-LLaMA-3-8B-R/raw/main/model.safetensors.index.json', 'model.safetensors.index.json', dir, onProgress)
    } else if (this.size === '70B') {
      const model = await fetchSave('https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-70B/resolve/main/model.safetensors.index.json?download=true', 'model.safetensors.index.json', dir, onProgress)
      await fetchSave('https://huggingface.co/bofenghuang/Meta-Llama-3-8B/resolve/main/original/tokenizer.model', 'tokenizer.model', dir, onProgress)
      for (const i of range(17)) {
        await fetchSave(`https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-70B/resolve/main/model-${(i + 1).toString().padStart(5, '0')}-of-000017.safetensors?download=true`, `model-${(i + 1).toString().padStart(5, '0')}-of-000017.safetensors`, dir, onProgress)
      }
      return model
