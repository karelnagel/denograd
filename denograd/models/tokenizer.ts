...
export class Tokenizer {
  ...
  static llama = async (path: string) => {
    ...
    return new Tokenizer(
      ...
      ranks,
      new Map(specialTokensList.map((token, i) => [token, ranks.size + i])),
    )
  }
  constructor(public pat: RegExp, public mergeable_ranks: Map<string, number>, public special_tokens: Map<string, number>) {
