...
import { all_int, all_same, ansilen, assert, cache, cache_fn, colored, dataclass, DEBUG, dedup, Enum, get_env, get_number_env, is_less_than, isinstance, isInt, product, range, round_up, set_default, to_function_name, USE_TC, zip } from '../helpers.ts'
import { can_pad, ge, graph_rewrite, GroupOp, gt, idiv, KernelInfo, le, mul, ne, Ops, print_uops, prod, resolve, sint, sum, UOp, Variable, view_left } from '../ops.ts'
...
export class Kernel {
  ...
  apply_opt = (opt: Opt, append_opt = true) => {
    ...
    if (opt.op === OptOps.SWAP) amt = opt.amt! // amt===an axis in the SWAPs
      ...
      check(isInt(amt) && amt !== 1, 'shift/padto of amt 1 or Node===meaningless')
    ...
    if (this.reduceop !== undefined && ([OptOps.GROUP, OptOps.GROUPTOP].includes(opt.op) || (this.group_for_reduces && ![OptOps.NOLOCALS, OptOps.PADTO].includes(opt.op)))) {
      ...
      const upcast_sz = prod(zip(this.full_shape.slice(this.first_upcast), this.sts[0].shape.slice(this.first_upcast)).filter(([a, b]) => a === b).map(([a, _]) => a as number))
      const local_sz = prod(this.full_shape.slice(this.first_reduce - this.local_dims, this.first_reduce + this.group_for_reduces)) as number
      const smem_sz = amt * acc_sz * upcast_sz * local_sz
      check(smem_sz <= this.opts.shared_max, `exceeds maximum shared memory size: needs ${smem_sz}, max ${this.opts.shared_max}`)
    }
  ...
  hand_coded_optimizations = () => {
    ...
    if (this.opts.has_local && get_number_env('MV', 1) !== 0 && (MV_BLOCKSIZE > 1 || MV_THREADS_PER_ROW > 1 || MV_ROWS_PER_THREAD > 1) && this.reduceop !== undefined && this.reduceop.arg[0] === Ops.ADD && this.full_shape.length >= 2 && this.opts.has_shared) {
      const mulop = this.reduceop.src[0]
      if (mulop.op === Ops.MUL && mulop.src[0].op === Ops.LOAD && mulop.src[1].op === Ops.LOAD) {
        ...
        if (strides0[this.first_reduce] === 1 && !(has_expanded_axis(st0.shape, strides0 as sint[]) && has_expanded_axis(st1.shape, strides1 as sint[]))) {
          for (const global_idx of range(this.global_dims)) {
            if ((this.full_shape[this.first_reduce] as number) % MV_THREADS_PER_ROW === 0 && (this.full_shape[global_idx] as number) % (MV_BLOCKSIZE * MV_ROWS_PER_THREAD) === 0) {
              if (DEBUG >= 3) console.log(`MATVEC: full_shape=${this.full_shape} first_reduce=${this.first_reduce} strides0=${strides0} MV_BLOCKSIZE=${MV_BLOCKSIZE} MV_THREADS_PER_ROW=${MV_THREADS_PER_ROW} MV_ROWS_PER_THREAD=${MV_ROWS_PER_THREAD}`)
    ...
    if (this.opts.has_local && this.opts.has_shared && all_int(this.sts[0].shape.slice(0, this.first_reduce))) {
      ...
      if (this.bufs[0].src[0].dtype.name.startsWith('image') && !this.float4_axis(0) && this.group_for_reduces && this.first_reduce <= 2 && prod(this.sts[0].shape) as number > 1) {
        const axes = this.sts[0].unit_stride_axes()
        if (axes.length !== 1) throw new Error(`wrong number of stride 1 axis : ${axes}`)
    ...
    for (const axis of range(this.first_reduce)) {
      ...
      if (isInt(this.full_shape[axis]) && this.full_shape[axis] as number <= 7 && this.sts.some((st) => st.axis_is_masked(axis)) && ((prod(this.full_shape.slice(this.first_upcast)) as number) * (prod(to_upcast.map((j) => this.full_shape[j])) as number) * (this.full_shape[axis] as number)) <= 7 * 7) {
    ...
    while (resolve(ge(prod(this.sts[0].shape.slice(0, this.first_reduce)), 1024))) {
      ...
      for (const [axis, upcast_amount] of product(range(this.first_reduce), [3, 4])) { // consider all the non reduce axes, and a 3 or 4 reduce
        ...
        if (!upcasted_axis.has(axis) && isInt(this.full_shape[axis]) && this.full_shape[axis] % upcast_amount === 0 && this.sts.entries().some(([buf_index, st]) => (st.views.at(-1)!.strides[axis] === 0 && this.upcasted_axis(buf_index).some((x) => x[1] === 0)))) {
          xb_choices.push([sum(this.sts.map((st) => Number(st.views.at(-1)!.strides[axis] as number > 0))), sum(this.sts.map((st) => st.views[-1].strides[axis] as number)), axis, upcast_amount])
        }
      }
      if (xb_choices.length) {
        // TODO: this might sort wrong
        xb_choices = xb_choices.toSorted((a, b) => is_less_than(a, b) ? -1 : 1)
    ...
    if (this.first_reduce < this.first_upcast && (prod(this.full_shape.slice(this.first_upcast)) as number <= 4 || !this.upcasted_axis(this.full_buf_index).some(([_, __, r]) => r)) && (this.upcasted === 0 || prod(this.full_shape.slice(-this.upcasted)) as number < 64)) {
    ...
    for (const splits of [4]) {
      if (this.upcasted === 0 && this.full_unupcasted_shape.length && this.full_unupcasted_shape.at(-1) as number % splits === 0) this.apply_opt(new Opt(OptOps.UPCAST, this.full_unupcasted_shape.length - 1, splits))
    }

    // # **** local groups ****

    if (this.opts.has_local) {
      if (get_env('NOLOCALS') && this.local_dims === 0 && !this.group_for_reduces) this.apply_opt(new Opt(OptOps.NOLOCALS))
      else {
        // prioritize making expand axes local
        const local_axis_ranking = range(this.full_shape.slice(0, this.first_reduce).length).map((axis) => [Number(range(this.sts.length).some((buf_index) => this.sts[buf_index].views.at(-1)!.strides[axis] === 0)), axis])
        const to_local: [number, number][] = []
        for (const [_, axis] of local_axis_ranking.toSorted((a, b) => is_less_than([-a[0], -a[1]], [-b[0], -b[1]]) ? -1 : 1)) {
          const local_size = prod(to_local.map(([_, sz]) => sz))
          const local_sz = [...(axis === 0 ? [32] : []), 16, 8, 4, 3, 2].filter((x) => this.full_shape[axis] as number % x === 0 && local_size * x <= 128).shift()
          if (local_sz !== undefined) to_local.push([axis, local_sz])
        }
        let deleted_shape = 0
        for (let [axis, local_sz] of to_local.slice(0, 3).toSorted((a, b) => is_less_than(a, b) ? -1 : 1)) {
