...
export class ShapeTracker {
  ...
  real_strides = (ignore_valid = false): (undefined | sint)[] => {
    ...
    const newidx = uopGivenValid(valid, idx)
  ...
  axis_is_masked = (axis: number): boolean => {
    ...
    return [...graphRewrite(valid, symbolicFlat).sparents().keys().filter((x) => x.op === Ops.RANGE).map((x) => x.arg[0])].includes(axis)
  }
  ...
  stride = (mul: number[]) => new ShapeTracker([...this.views.slice(0, -1), this.views.at(-1)!.stride(mul)])
