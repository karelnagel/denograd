...
class SimpleMathTrait {
    alu = (arg: Ops, src: any): typeof this => {
        throw new Error('Not implemented')
    }
    constLike = (b: ConstLike): typeof this => {
        throw new Error('Not implemented')
    }

    //   # great functions you get!
    ufix = (x: any) => (x instanceof MathTrait) ? x : this.constLike(x)
    _binop = (op: any, x: any, reverse: boolean) => reverse ? this.ufix(x).alu(op, this) : this.alu(op, this.ufix(x))
    ...
    ne = (x: any) => this.alu(Ops.CMPNE, this.ufix(x))
...
export type ConstLike = ConstType | ConstType[]
