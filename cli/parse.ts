...
class ZodEnum<T extends string> extends Type<T> {
  ...
  _parse = (value: any) => {
    if (!this.options.includes(value)) throw new Error(`Invalid enum, received: ${value}, options: ${this.options}`)
  }
}
...
export const z = { number: () => new ZodNumber(), enum: <T extends string>(options: T[]) => new ZodEnum(options), string: () => new ZodString(), boolean: () => new ZodBoolean() }
...
export const parse = <T extends Schema>(value: any, schema: T): { success: true; data: ParsedSchema<T> } | { success: false; errors: ParsingError[] } => {
  const errors: ParsingError[] = []
  if (value === undefined) errors.push({ message: `Can't parse, value is undefined`, path: [] })
  const data: any = {}
