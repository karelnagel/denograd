...
const help = (schema: Schema): string => {
  ...
  for (const [k, v] of Object.entries(schema)) {
    lines.push([
      ...
      `${v.description || ""}${
        v.defaultValue
          ? colored(` (default: ${v.defaultValue})`, "yellow")
          : v.required
          ? colored(` (required)`, "red")
          : ""
      }`,
    ]);
  }
  const maxLengths = lines[0].map((_, i) =>
    Math.max(...lines.map((line) => line[i].length))
  );
  res +=
    lines.map((line) =>
      line.map((line, i) => line.padEnd(maxLengths[i] + 2)).join("")
    ).join("\n") + "\n";
  return res;
};
...
export const parseArgs = <T extends Schema>(schema: T): ParsedSchema<T> => {
  ...
  console.log(
    res.errors.map((x) =>
      colored(`Error with '${x.path.join(".")}': ${x.message}`, "red")
    ).join("\n") + "\n\n" + help(schema),
  );
